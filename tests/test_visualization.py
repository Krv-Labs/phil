...
class TestPlotMds:
    ...
    def test_single_descriptor_raises(self):
        ...
        with pytest.raises(ValueError, match="at least two descriptors"):
            plot_mds(descriptors, selected_index=0, random_state=0)

    def test_invalid_selected_index_raises(self):
        descriptors = self._make_descriptors(n=3)
        with pytest.raises(ValueError, match="selected_index is out of range"):
            plot_mds(descriptors, selected_index=3, random_state=0)

    def test_mismatched_descriptor_shapes_raises(self):
        descriptors = [np.zeros((2, 2)), np.zeros((3, 2))]
        with pytest.raises(ValueError, match="same flattened length"):
            plot_mds(descriptors, selected_index=0, random_state=0)
